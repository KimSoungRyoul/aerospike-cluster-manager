...
function sanitizeConditions(value: unknown): FilterCondition[] {
  ...
  return value.flatMap((condition, index) => {
    ...
    if (
      !isValidOperator(candidate.operator) ||
      !isValidBinType(candidate.binType)
    ) {
      return []
    }
...
export function serializeRecordFilters(
  filters: RecordFilterRouteState | undefined,
) {
...
export function buildDefaultReturnTo(
  connId: string,
  namespace: string,
  setName: string,
) {
